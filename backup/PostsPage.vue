...
<template>
  <v-ons-page>
    ...
    <div class="">
      <router-link v-for="(item) in posts" :to="`/posts/${item.slug}`" :key="item.ID" class="link-card">
        <v-ons-card class="post post-item">
          <v-ons-row>
            <v-ons-col :style="`background-image: url(${item.post_thumbnail_small});`" class="left"></v-ons-col>
