...
<template>
  <div class="contact-item">
    <div class="info" @click="toggleDetail(item)">
      ...
      <span class="see-more">
        <v-ons-icon v-if="!item.showDetail" icon="fa-search-plus"></v-ons-icon>
        <v-ons-icon v-if="item.showDetail" icon="fa-search-minus"></v-ons-icon>
      </span>
    </div>
    <div v-if="item.showDetail" class="detail">
      <ul>
      ...
      if (typeof sms === "undefined") {
        ...
        if (number !== null) {
          sms.send(
            ...
            options,
            success => {
              this.$ons.notification.toast(`SMS sent to ${item.displayName}!`, {
                timeout: 2000
              });
              console.log(success);
            },
            error => {
              this.$ons.notification.toast("Error sending SMS", {
                timeout: 2000
              });
              console.log(error);
            }
          ...
          this.$ons.notification.alert(
            "This contact doesn't have phone number"
          );
        }
      }
    }
