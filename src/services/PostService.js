...
const APIURL =
  "https://public-api.wordpress.com/rest/v1.1/sites/dailykorean.wordpress.com";
const POSTFIELD = {
  LIST:
    "ID,slug,title,author,date,short_URL,excerpt,categories,attachments,tags,post_thumbnail",
  DETAIL:
    "ID,slug,title,author,date,short_URL,excerpt,categories,attachments,tags,post_thumbnail,content"
};
const PERPAGE = 5;
...
export default {
  getList(page = 1) {
    return axios
      .get(
        `${APIURL}/posts?page=${page}&number=${PERPAGE}&fields=${
          POSTFIELD.LIST
        }`
      )
      .then(response => {
  ...
  getDetail(slug) {
    return axios
      .get(`${APIURL}/posts/slug:${slug}/?fields=${POSTFIELD.DETAIL}`)
      .then(response => {
        let thumbnail = Config.DEFAULT.IMAGE;
