...
let text = {
  ...
  de: {
    ...
    errorPasswordsDifferent:
      "Du hast zwei unterschiedliche Passwörter eingegeben.",
    firebaseErrors: {
...
export default {
  ...
  mounted() {
    ...
    if (
      (!this.$root.loginRequiringPagesOnStart &&
        !this.$root.config.loginRequiredForAllPages) ||
      this.$root.user
    ) {
      ...
      if (navigator.onLine === false) {
        ...
        window.firebase
          ...
          .catch(err => {
            ...
            // Show error alert
            window.f7.alert(
              this.text.firebaseErrors[err.code],
              this.text.error
            );
          });
      }
    ...
    handleSignInDone: function() {
      ...
      this.$f7.views.forEach(view => {
        if (loginRequiringPages[view.selector]) {
          this.$nextTick(() => {
            view.router.load({
              url: loginRequiringPages[view.selector],
              animatePages: false
            });
          });
        }
