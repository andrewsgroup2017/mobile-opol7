  ...
  methods: {
      ...
      if (file) {
        ...
        window
          ...
          .catch(() => {
            ...
            window.f7.alert(
              "Cannot upload the photo :-(<br />Please try again later",
              "Trouble with Firebase"
            );
          });
      } else {
        window.f7.addNotification({
    ...
    handlePhotoUploadFromDevice: function(type) {
      ...
      if (navigator.camera && window.resolveLocalFileSystemURL) {
            ...
            window.resolveLocalFileSystemURL(imageUri, function(fileEntry) {
              fileEntry.file(function(file) {
                ...
                reader.onloadend = function() {
                  let blob = new window.Blob([new Uint8Array(this.result)], {
                    type: file.type
                  });
                  window
                    .store(self.store)
          ...
          },
          {
            sourceType:
              window.Camera.PictureSourceType[
                type === "gallery" ? "PHOTOLIBRARY" : "CAMERA"
              ],
        ...
        window.f7.alert(
          'Please add "cordova-plugin-camera" to useCordovaPlugins option in the configuration file.'
        );
        // File plugin not available
      } else {
        window.f7.alert(
          'Please add "cordova-plugin-file" to useCordovaPlugins option in the configuration file.'
        );
      }
    },
      ...
      if (this.db !== "") {
        ...
        window
          ...
          .then(url => {
            ...
            window
              ...
              .catch(() => {
                window.f7.hideIndicator();
                window.f7.alert(
                  "Cannot update the photo url :-(<br />Please try again later",
                  "Trouble with Firebase"
                );
              });
          })
          .catch(() => {
            window.f7.hideIndicator();
            window.f7.alert(
              "Cannot load the photo url :-(<br />Please try again later",
              "Trouble with Firebase"
            );
          });
      } else {
